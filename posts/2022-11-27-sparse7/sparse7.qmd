...
You can see that there is some good speedup for large matrices, but for smaller ones we aren't getting much. It's also worth noting that the function is being recompiled every time the structure of `A` changes. The essentially consstant time for the JIT'd version strongly suggests that for this problem size most of the time is spent on bookkeeping and memory stuff. 