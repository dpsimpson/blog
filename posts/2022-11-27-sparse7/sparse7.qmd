...
### Attempt 3: A desperate attempt to make this
...
## The numerical factorisation

Finally we have made it here!



<!-- Let's discover how fast `jnp.lexsort` is!
...
#| eval: false
@partial(jit, static_argnums = (5, 6))
def symbolic_cholesky3(A_indices_, A_indptr_, L_indptr, row_count, parent, nnz, max_row):
  print("compiling symb3")

  ## innermost while loop
  def body_while(val):
    which_col, which_row, i, counter,  node, col_ptr, mark = val
    #print(f"{counter}: ({node}-> {parent[node]}), {i} , {type(node)} & {cond_while(val)}")
    mark = mark.at[node].set(i)
    which_row = which_row.at[counter].set(node.copy())
    which_col = which_col.at[counter].set(i)
    col_ptr = col_ptr.at[node].add(1)
    inext = parent[node]
    return (which_col, which_row, i, counter + 1, inext, col_ptr, mark)
  ...
  ## Inner for loop
  def body_inner_for(indptr, val):
     which_col, which_row, i, counter, col_ptr, mark = val
     node = A_indices_[indptr]
     
     which_col, which_row, i, counter,  node, col_ptr, mark = lax.while_loop(cond_while, body_while, (which_col, which_row, i, counter,  node, col_ptr, mark))
     return (which_col, which_row, i, counter, col_ptr, mark)
  
  ## Outer for loop
  def body_out_for(i, val):
     which_col, which_row, counter, L_indices, col_ptr, mark = val
     mark = mark.at[i].set(i)
     which_row = which_row.at[counter].set(i)
     which_col = which_col.at[counter].set(i)
     counter = counter+1

     which_col, which_row, i, counter, col_ptr, mark = lax.fori_loop(A_indptr_[i], A_indptr_[i+1], body_inner_for, (which_col, which_row, i, counter, col_ptr, mark))

     return (which_col, which_row, counter, L_indices, col_ptr, mark)
  ...
  which_col = jnp.ones(nnz, dtype=int) *(-1)
  which_row = jnp.ones(nnz, dtype=int)* (-1)
  counter = 0

  init = (which_col, which_row, counter, L_indices, col_ptr, mark)
  which_col, which_row, counter, L_indices, col_ptr, mark = lax.fori_loop(0, n, body_out_for, init)
  return (which_col[jnp.lexsort((which_col, which_row))], counter, which_col, which_row)
...
L_indptr = np.zeros(A.shape[0]+1, dtype=int)
L_indptr[1:] = np.cumsum(col_count)
L_indices, counter, which_col, which_row = symbolic_cholesky3(A.indices, A.indptr, L_indptr, row_count, parent, nnz = int(L_indptr[-1]), max_row = int(max(row_count)))
L_indices_true, L_indptr_true = _symbolic_factor(A_indices, A_indptr)
print(all(L_indices == L_indices_true))
print(all(L_indptr == L_indptr_true))
...
L_indptr = np.zeros(A.shape[0]+1, dtype=int)
L_indptr[1:] = np.cumsum(col_count)
times = timeit.repeat(lambda:symbolic_cholesky3(A.indices, A.indptr, L_indptr, row_count, parent, nnz = int(L_indptr[-1]), max_row = int(max(row_count))),number = 1, repeat = 5)
print(f"n = {A.shape[0]}: {[round(t,2) for t in times]}")
...
Well thank Christ for that. I mean, it's not perfect but I've definitely seen worse! A quick compare with the pure python implementation suggests that we are doing ok.


```{python}
#| echo: false
#} eval: false
def symbolic_cholesky3_base(A_indices, A_indptr, L_indptr, row_count, parent, nnz):

  n = len(A_indptr) - 1
  col_ptr = L_indptr + 1
  L_indices = np.zeros(L_indptr[-1], dtype=int)
  mark = [-1] * n
  which_col = np.zeros(nnz, dtype=int)
  which_row = np.zeros(nnz, dtype=int)
  counter = 0
  ...
  for i in range(n):
    ...
    for indptr in range(A_indptr[i], A_indptr[i+1]):
      ...
      while node < i and mark[node] != i:
        mark[node] = i
        which_row[counter] = node
        which_col[counter] = i
        counter = counter + 1
        node = parent[node]
  
  return which_col[np.array(np.lexsort((which_col, which_row)))]
...
L_indices = symbolic_cholesky3_base(A.indices, A.indptr, L_indptr, row_count, parent, L_indptr[-1])
L_indices_true, L_indptr_true = _symbolic_factor(A_indices, A_indptr)
...
L_indptr = np.zeros(A.shape[0]+1, dtype=int)
L_indptr[1:] = np.cumsum(col_count)
times = timeit.repeat(lambda:symbolic_cholesky3_base(A.indices, A.indptr, L_indptr, row_count, parent, nnz = int(L_indptr[-1])),number = 1, repeat = 1)
print(f"n = {A.shape[0]}: {[round(t,2) for t in times]}")

```

Ok. So that works well enough. I'm not thrilled^[It's still much slower than pure python.] with it, but it at least solves the problem and does not have catastrophic scaling. 

That said, I was talking to Junpeng and he suggested I try an `jnp.where` rather than all of those damn `.at`s. So let's see how that compares. Writing this blog, as I do, sequentially, I have suspicions^[Junpeng is a professional JAX-er. I'm unemployed. We are not the same.]. I've might have wasted^[I'm on a flight from Melbourne to New York right now so I promise you there is nothing better that I could be doing. I'm also drinking.] my time.
